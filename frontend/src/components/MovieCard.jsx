...
//     setInList(isInWishlist(movie.id));
//   }, [movie]);
...
//         poster: movie.poster, // fallback if you use local poster
//       });
//       setInList(true);
//     }
//   };
...
//         className="absolute top-2 right-2 bg-black/60 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:scale-110 focus:opacity-100"
//         style={{
//           zIndex: 50,
//           display: 'flex',
//           alignItems: 'center',
...
const MovieCard = ({ movie, width = 150, imageSize = 'w300' }) => {
  ...
  const toggleWishlist = (e) => {
    // prevent card click (which navigates)
    e.stopPropagation();
    ...
    // no need to do anything else — the wishlist util will notify subscribers
  };
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={inList ? `Remove ${movie.title || movie.name} from wishlist` : `Add ${movie.title || movie.name} to wishlist`}
        title={inList ? 'Remove from wishlist' : 'Add to wishlist'}
        className="absolute top-2 right-2 bg-black/60 p-2 rounded-full opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity duration-300 hover:scale-110"
        style={{
          zIndex: 50,
          display: 'flex',
          alignItems: 'center',
